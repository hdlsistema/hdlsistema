...
create or replace function public.access_pass_effective_valid_until(
  p_access_pass_id uuid,
  p_stored_valid_until timestamptz
)
returns timestamptz
language plpgsql
security definer
...
begin
  select coalesce(
    ticket_event.end_at,
    reservation_event.end_at,
    slot.end_at,
    case
      when r.reservation_type = 'cabin' and r.check_out is not null
        then (r.check_out::text || 'T12:00:00-06:00')::timestamptz
      else null
    end,
    case
      when r.reservation_date is not null
        then (r.reservation_date::text || 'T' || coalesce(to_char(r.reservation_time, 'HH24:MI'), '00:00') || ':00-06:00')::timestamptz
      else null
    end,
    ticket_event.start_at,
    reservation_event.start_at,
    slot.start_at
  )
  into v_anchor
  from public.access_passes ap
  left join public.reservations r on r.id = ap.reservation_id
  left join public.experience_slots slot on slot.id = r.experience_slot_id
  left join public.events reservation_event on reservation_event.id = r.event_id
  left join public.event_ticket_types ticket_type on ticket_type.id = coalesce(ap.event_ticket_type_id, r.event_ticket_type_id)
  left join public.events ticket_event on ticket_event.id = ticket_type.event_id
  where ap.id = p_access_pass_id
  limit 1;

  if v_anchor is null then
    return p_stored_valid_until;
  end if;

  v_policy_until := v_anchor + interval '12 hours';

  if p_stored_valid_until is null then
    return v_policy_until;
  end if;

  return least(p_stored_valid_until, v_policy_until);
end;
$$;
...
with computed as (
  select
    ap.id,
    public.access_pass_effective_valid_until(ap.id, ap.valid_until) as effective_valid_until
  from public.access_passes ap
  where ap.reservation_id is not null or ap.event_ticket_type_id is not null
)
update public.access_passes ap
set
  valid_until = computed.effective_valid_until,
  metadata = jsonb_set(
    coalesce(ap.metadata, '{}'::jsonb),
    '{qrExpiryPolicy}',
    jsonb_build_object('hoursAfterEvent', 12, 'source', '073_entry_qr_expiry_12h'),
    true
  )
from computed
where ap.id = computed.id
  and computed.effective_valid_until is not null
  and (ap.valid_until is null or ap.valid_until > computed.effective_valid_until);
...
begin
  ...
  v_effective_valid_until := public.access_pass_effective_valid_until(v_pass.id, v_pass.valid_until);
  if (v_pass.valid_from is not null and v_pass.valid_from > now())
    or (v_effective_valid_until is not null and v_effective_valid_until < now()) then
  ...
  perform public.write_transaction_audit(v_actor_id, 'checkin_registered', 'checkins', v_checkin_id, null,
    jsonb_build_object('pass_number', v_pass.pass_number, 'accessType', v_access_type, 'validUntil', v_effective_valid_until));
  return v_checkin_id;
end;
$$;

revoke all on function public.access_pass_effective_valid_until(uuid, timestamptz) from public, anon;
...
grant execute on function public.access_pass_effective_valid_until(uuid, timestamptz) to authenticated;
