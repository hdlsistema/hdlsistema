...
begin
  if to_regclass('public.customer_addresses') is null
    or to_regclass('public.order_shipping_addresses') is null then
    raise exception 'Migration 039 must be applied before migration 049';
  end if;
  ...
  if exists (
    ...
    where deleted_at is null
      and (
        nullif(btrim(label), '') is null
        or nullif(btrim(recipient_name), '') is null
        or nullif(btrim(phone), '') is null
        or nullif(btrim(email::text), '') is null
        or nullif(btrim(line1), '') is null
        or nullif(btrim(line2), '') is null
        or nullif(btrim(neighborhood), '') is null
        or nullif(btrim(city), '') is null
        or nullif(btrim(state), '') is null
        or nullif(btrim(postal_code), '') is null
        or nullif(btrim(country), '') is null
        or nullif(btrim("references"), '') is null
      )
  ) or exists (
    select 1
    from public.order_shipping_addresses
    where nullif(btrim(label), '') is null
      or nullif(btrim(recipient_name), '') is null
      or nullif(btrim(phone), '') is null
      or nullif(btrim(email::text), '') is null
      or nullif(btrim(line1), '') is null
    ...
    raise exception 'Incomplete shipping addresses must be corrected before migration 049';
  end if;
end $$;
...
alter table public.customer_addresses
  drop constraint if exists customer_addresses_all_delivery_fields_check,
  add constraint customer_addresses_all_delivery_fields_check check (
    length(btrim(label)) >= 1
    and length(btrim(recipient_name)) >= 2
    and length(btrim(phone)) >= 7
    and position('@' in btrim(email::text)) > 1
    and length(btrim(line1)) >= 4
    and length(btrim(line2)) >= 1
    and length(btrim(neighborhood)) >= 1
    and length(btrim(city)) >= 2
    and length(btrim(state)) >= 2
    and length(btrim(postal_code)) >= 4
    and length(btrim(country)) >= 2
    and length(btrim("references")) >= 1
  );

alter table public.order_shipping_addresses
  drop constraint if exists order_shipping_addresses_all_delivery_fields_check,
  add constraint order_shipping_addresses_all_delivery_fields_check check (
    length(btrim(label)) >= 1
    and length(btrim(recipient_name)) >= 2
    and length(btrim(phone)) >= 7
    and position('@' in btrim(email::text)) > 1
...
create or replace function public.enforce_complete_order_shipping_address()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if new.requires_shipping
    ...
    if not exists (
      ...
      where osa.order_id = new.id
        and length(btrim(osa.label)) >= 1
        and length(btrim(osa.recipient_name)) >= 2
        and length(btrim(osa.phone)) >= 7
        and position('@' in btrim(osa.email::text)) > 1
        and length(btrim(osa.line1)) >= 4
        and length(btrim(osa.line2)) >= 1
        and length(btrim(osa.neighborhood)) >= 1
        and length(btrim(osa.city)) >= 2
        and length(btrim(osa.state)) >= 2
        and length(btrim(osa.postal_code)) >= 4
        and length(btrim(osa.country)) >= 2
        and length(btrim(osa."references")) >= 1
    ) then
...
create constraint trigger orders_require_complete_shipping_address
after insert or update on public.orders
deferrable initially deferred
for each row
execute function public.enforce_complete_order_shipping_address();

insert into public.audit_logs (actor_user_id, action, entity_type, entity_id, after_data)
values (
  null,
  'migration_049_required_shipping_address_checkout',
  'system',
  gen_random_uuid(),
  jsonb_build_object('status', 'applied', 'scope', 'atomic_shipping_checkout')
);
