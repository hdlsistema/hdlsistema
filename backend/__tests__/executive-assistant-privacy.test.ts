...
describe('executive assistant privacy and access contract', () => {
  it('keeps precise operational lookups local and excludes raw sensitive payloads', () => {
    ...
    expect(service).toContain('answerCustomerDetailQuestion')
    expect(service).toContain('answerLogisticsQuestion')
    expect(service).toContain('answerPaymentsDetailQuestion')
    expect(service).toContain('answerInventoryDetailQuestion')
    expect(service).toContain('answerCampaignDetailQuestion')
    expect(service).toContain('Revisé datos reales de Eventos/Experiencias, Reservaciones, Tipos de boleto, Pases QR y Check-ins')
    expect(service).toMatch(/const preciseAnswer = await answerPreciseLocalQuestion\(payload\.message, payload\.history\)[\s\S]*return \{ answer: plainAiResponse\(preciseAnswer\)/)
    expect(service).toContain('consultas puntuales se resuelven localmente')
    expect(service).toContain('No muestres IDs técnicos, metadata cruda, tokens, hashes ni payloads de proveedor')
