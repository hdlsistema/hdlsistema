...
describe('flujo punta a punta de boletos y horarios comerciales', () => {
  ...
  it('cotiza letrero luminoso en cena romantica desde app, control y Supabase', () => {
    ...
    expect(reservationScreen).toContain('romanticSign: romanticSignSelection')
    expect(reservationScreen).toContain("acceptedContractMetadata(activeContractTerms, 'mobile_experience_reservation')")
    expect(reservationScreen).toContain('metadata: Object.keys(metadata).length > 0 ? metadata : undefined')
  ...
  it('emite QR publico solo para accesos y conserva consumo protegido', () => {
    ...
    const entryOnly = readFileSync(resolve(__dirname, '../migrations/062_access_qr_only_for_entries.sql'), 'utf8')
    const routes = readFileSync(resolve(__dirname, '../src/modules/checkin/checkin.routes.ts'), 'utf8')

    expect(issuer).toContain('ensureEventTicketAccessPassesForPaidOrder')
    expect(issuer).toContain('ensureReservationAccessPassForPaidOrder')
    expect(issuer).not.toContain('ensurePaidOrderCredentialForPaidOrder')
    expect(issuer).not.toContain('purchase_credential')
    ...
    expect(entryOnly).toContain('ORDER_ACCESS_PASS_NOT_ALLOWED')
    expect(entryOnly).toContain("metadata ->> 'accessType' in ('wine_order', 'paid_order')")
  })
})
