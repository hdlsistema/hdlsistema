...
describe('persistencia del Centro de Control', () => {
  ...
  it('aplica alcances por sede a restaurantes, reservaciones y control de entradas', () => {
    const permissions = readFileSync(resolve(root, '../backend/src/modules/admin/controlPermissions.ts'), 'utf8')
    const dataScope = readFileSync(resolve(root, '../backend/src/modules/admin/controlDataScope.ts'), 'utf8')
    const reservations = readFileSync(resolve(root, '../backend/src/modules/reservations/reservations.service.ts'), 'utf8')
    const checkin = readFileSync(resolve(root, '../backend/src/modules/checkin/checkin.service.ts'), 'utf8')
    const commercial = readFileSync(resolve(root, '../backend/src/modules/commercial/commercial.service.ts'), 'utf8')

    expect(permissions).toContain("label: 'Todas las sedes'")
    expect(permissions).toContain("label: 'Restaurante Teodoro'")
    expect(permissions).toContain("label: 'Restaurante Nieto'")
    expect(dataScope).toContain("restaurante_teodoro")
    expect(dataScope).toContain("restaurante_nieto")
    expect(dataScope).toContain('textLooksNieto')
    expect(reservations).toContain('controlDataScopeReservationOrFilter')
    expect(reservations).toContain('assertControlDataScopeRecord')
    expect(checkin).toContain('controlDataScopeAccessPassOrFilter')
    expect(checkin).toContain('controlDataScopeAccessPassIds')
    expect(commercial).toContain('filterRowsByControlDataScope')
    expect(commercial).toContain('isRestaurantOnlyDataScope')
    expect(commercial).toContain('restaurantOnly ? [] : cabinRows')
    expect(commercial).toContain('restaurantOnly ? [] : spaceRows')
  })
})
