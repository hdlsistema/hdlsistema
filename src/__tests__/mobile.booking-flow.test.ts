...
describe('flujo mobile de reservaciones y boletos', () => {
  ...
  it('muestra campana conectada y crea el canal nativo de notificaciones', () => {
    ...
    expect(push).toContain("const nativeDeepLink = deepLink.replace(/^\\/app(?=\\/|$)/, '') || '/'")
    expect(push).toContain("nativePath !== '/control' && !nativePath.startsWith('/control/')")
    expect(push).toContain('window.location.assign(nativeDeepLink)')
