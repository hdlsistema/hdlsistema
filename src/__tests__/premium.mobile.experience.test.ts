...
describe('premium customer app experience', () => {
  ...
  it('publica el domicilio oficial con marcador Hacienda y accesos a Maps y Waze', () => {
    ...
    const locations = readFileSync(resolve(__dirname, '../app/utils/officialLocations.ts'), 'utf8')
    const directions = readFileSync(resolve(__dirname, '../app/components/mobile/OfficialDirectionsSheet.tsx'), 'utf8')

    expect(locations).toContain('Teodoro Olivares S/N, 20668 San Luis de Letras, Ags.')
    expect(locations).toContain('https://www.google.com/maps/dir/')
    expect(locations).toContain('https://www.waze.com/ul?')
    expect(directions).toContain('OfficialDirectionsSheet')
    expect(map).toContain("poi.slug === 'vinedos-bodegas-hacienda-de-letras'")
    expect(map).toContain("? 'estate' as const")
    expect(map).toContain("poi.slug === 'cabanas-hacienda-de-letras'")
    expect(map).toContain("? 'lodging' as const")
