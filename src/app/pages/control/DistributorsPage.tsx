...
export function DistributorsPage() {
  ...
  return <div className="control-page control-page--distributors min-w-0 space-y-6">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><SectionTitle eyebrow="Venta B2B" title="Distribuidores" subtitle="Socios comerciales, condiciones, crédito y pedidos mayoristas." /><div className="control-action-bar xl:justify-end"><button type="button" onClick={load} className={secondary}><RefreshCw size={15} />Actualizar</button><button type="button" disabled={!writable || distributors.length === 0} onClick={() => setModal('order')} className={secondary}><ShoppingBag size={15} />Nuevo pedido</button><button type="button" disabled={!writable} onClick={() => setModal('distributor')} className={primary}><Plus size={15} />Nuevo distribuidor</button></div></div>
...
function Submit({ saving, label }: { saving: boolean; label: string }) { return <div className="control-form-actions md:col-span-2"><button type="submit" disabled={saving} className={primary}>{saving ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />}{label}</button></div> }
