...
export function LodgingPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const submitUnit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setModal(null)
      setToast('Cabaña agregada al inventario hotelero.')
      await load()
    } catch (err) {
  ...
  const submitCheckIn = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setOperationNotes('')
      setModal(null)
      setToast('Check-in hotelero registrado.')
      await load()
    } catch (err) {
  ...
  return (
    <div className="control-page min-w-0 space-y-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        {embedded
          ? <div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--color-gold)]">PMS hotelero</p><h2 className="mt-1 text-2xl font-semibold text-[var(--color-ink)]">Hotel / Cabañas</h2><p className="mt-1 text-sm text-[var(--color-muted)]">Inventario físico, venta por noche, recepción y housekeeping en una sola vista.</p></div>
          : <SectionTitle eyebrow="Operación hotelera" title="Hospedaje y disponibilidad" subtitle="Inventario por cabaña, bloqueos nocturnos, llegadas, salidas y limpieza." />}
      ...
      <section aria-label="Gráficas hoteleras" className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
      ...
      <section className="rounded-xl border border-[var(--color-line)] bg-[var(--color-panel)] p-4 shadow-[var(--shadow-card)]">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <p className="pb-2 text-xs text-[var(--color-muted)]">Las noches se manejan en intervalo entrada inclusiva / salida exclusiva, como un PMS hotelero.</p>
        </div>
      </section>
      ...
      <ControlConfirmDialog open={Boolean(releaseEntry)} title="Liberar bloqueo hotelero" message={releaseEntry ? `${entryLabel(releaseEntry)} dejará de bloquear ${releaseEntry.unitCode} del ${dateOnly(releaseEntry.startDate)} al ${dateOnly(releaseEntry.endDate)}.` : ''} confirmLabel="Liberar fechas" busy={saving} onCancel={() => setReleaseEntry(null)} onConfirm={confirmReleaseEntry} />
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) {
  return <div className="fixed inset-0 z-[150] flex items-center justify-center bg-[#210711]/68 p-4 backdrop-blur-sm"><button type="button" aria-label="Cerrar" onClick={onClose} className="absolute inset-0" /><section className="relative z-10 max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-[var(--color-line)] bg-[var(--color-page)] p-5 shadow-[0_35px_90px_rgba(29,5,12,0.38)]"><header className="mb-5 flex items-center justify-between"><h2 className="text-xl font-semibold text-[var(--color-burgundy)]">{title}</h2><button type="button" onClick={onClose} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[var(--color-line)] bg-white"><X size={16} /></button></header>{children}</section></div>
}
