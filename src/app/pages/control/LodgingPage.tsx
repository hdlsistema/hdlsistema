  ...
  return (
    <div className="control-page min-w-0 space-y-5">
      ...
      <section aria-label="Gráficas de ocupación de cabañas" className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
        <article className="rounded-xl border border-[var(--color-line)] bg-[var(--color-panel)] p-5 shadow-[var(--shadow-card)]">
          ...
          <div className="mt-5 flex items-center gap-5">
            <div className="grid h-32 w-32 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(#5B0B1F 0 ${calendarSummary.occupancy}%, rgba(37,47,55,0.12) ${calendarSummary.occupancy}% 100%)` }}><div className="grid h-[92px] w-[92px] place-items-center rounded-full bg-[var(--color-panel)] text-center"><div><p className="text-3xl font-semibold text-[var(--color-ink)]">{calendarSummary.occupancy}%</p><p className="text-[9px] uppercase text-[var(--color-muted)]">ocupación</p></div></div></div>
            <div className="space-y-2 text-xs"><Legend color="#5B0B1F" label={`${calendarSummary.totals.reserved} reservadas`} /><Legend color="#d3a347" label={`${calendarSummary.totals.holds} en hold`} /><Legend color="#b85f54" label={`${calendarSummary.totals.blocked} bloqueadas`} /><Legend color="#252F37" label={`${calendarSummary.totals.available} disponibles`} /></div>
          </div>
          ...
          <div className="mt-5 overflow-x-auto pb-2">
            <div className="flex min-w-max items-end gap-3">
              {calendarSummary.byDay.map((item) => {
                ...
                return <div key={`summary-${item.day}`} className="w-14 text-center"><div className="flex h-32 flex-col-reverse overflow-hidden rounded-lg bg-[var(--color-soft)]" title={`${item.available} disponibles · ${item.reserved} reservadas · ${item.holds} holds · ${item.blocked} bloqueadas`}><span style={{ height: `${(item.available / total) * 100}%` }} className="bg-[#252F37]" /><span style={{ height: `${(item.reserved / total) * 100}%` }} className="bg-[#5B0B1F]" /><span style={{ height: `${(item.holds / total) * 100}%` }} className="bg-[#d3a347]" /><span style={{ height: `${(item.blocked / total) * 100}%` }} className="bg-[#b85f54]" /></div><p className="mt-2 text-[9px] font-semibold uppercase text-[var(--color-muted)]">{shortDay(item.day)}</p><p className="text-[9px] text-[#252F37]">{item.available} libres</p></div>
              })}
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2"><Legend color="#252F37" label="Disponible" /><Legend color="#5B0B1F" label="Reservada" /><Legend color="#d3a347" label="Hold pendiente" /><Legend color="#b85f54" label="Bloqueada" /></div>
        </article>
      </section>
      ...
      <section className="overflow-hidden rounded-xl border border-[var(--color-line)] bg-[var(--color-panel)] shadow-[var(--shadow-card)]">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--color-line)] px-4 py-3"><div><h2 className="text-base font-semibold">Calendario por cabaña</h2><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1"><Legend color="#252F37" label="Disponible" /><Legend color="#5B0B1F" label="Reservada" /><Legend color="#d3a347" label="Hold" /><Legend color="#b85f54" label="Bloqueada / fuera de servicio" /></div></div><span className="text-xs text-[var(--color-muted)]">{days.length} noches visibles</span></header>
            ...
            {!loading && units.map((unit) => (
              <div key={unit.id} className="contents">
                ...
                {days.map((day) => {
                  ...
                  const unavailable = unit.operationalStatus !== 'active' || unit.housekeepingStatus === 'out_of_service'
                  return <div key={`${unit.id}-${day}`} className="min-h-16 border-b border-r border-[var(--color-line)] p-1.5">{entry ? <button type="button" title={`${entryLabel(entry)} · ${entry.startDate} a ${entry.endDate}`} onClick={() => !entry.reservationId && writable ? setReleaseEntry(entry) : undefined} className={`h-full min-h-12 w-full rounded-md px-2 py-1 text-left text-[10px] font-semibold ${entry.entryType === 'reservation' ? 'bg-[#ead9df] text-[#5B0B1F]' : entry.entryType === 'hold' ? 'bg-[#fff0ce] text-[#8a5b13]' : 'bg-[#f3e1df] text-[#8b4038]'}`}><span className="line-clamp-2">{entryLabel(entry)}</span>{entry.entryType === 'hold' && entry.expiresAt ? <span className="mt-0.5 block text-[9px] font-normal">vence {new Intl.DateTimeFormat('es-MX', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Mexico_City' }).format(new Date(entry.expiresAt))}</span> : null}</button> : unavailable ? <span className="flex h-full min-h-12 items-center justify-center rounded-md bg-[#f3e1df] px-2 text-center text-[9px] font-semibold text-[#8b4038]">Fuera de venta</span> : <span className="flex h-full min-h-12 items-center justify-center rounded-md bg-[rgba(37,47,55,0.08)] text-[9px] font-semibold text-[#252F37]">Disponible</span>}</div>
                })}
              </div>
            ))}
          </div>
      ...
      {toast ? <div className="fixed bottom-6 right-6 z-[180] rounded-xl border border-[rgba(37,47,55,0.24)] bg-white px-4 py-3 text-sm font-semibold text-[#252F37] shadow-xl">{toast}<button type="button" onClick={() => setToast('')} className="ml-3"><X size={14} /></button></div> : null}
    </div>
  )
}
