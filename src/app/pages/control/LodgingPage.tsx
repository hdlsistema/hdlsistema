  ...
  return (
    <div className="control-page min-w-0 space-y-5">
          ...
          <div className="mt-5 overflow-x-auto pb-2">
            <div className="flex min-w-max items-end gap-3">
              {calendarSummary.byDay.map((item) => {
                ...
                return <div key={`summary-${item.day}`} className="w-14 text-center"><div className="flex h-32 flex-col-reverse overflow-hidden rounded-lg bg-[var(--color-soft)]" title={`${item.available} disponibles · ${item.reserved} reservadas · ${item.holds} holds · ${item.blocked} bloqueadas`}><span style={{ height: `${(item.available / total) * 100}%` }} className="bg-[#252F37]" /><span style={{ height: `${(item.reserved / total) * 100}%` }} className="bg-[#681126]" /><span style={{ height: `${(item.holds / total) * 100}%` }} className="bg-[#d3a347]" /><span style={{ height: `${(item.blocked / total) * 100}%` }} className="bg-[#b85f54]" /></div><p className="mt-2 text-[9px] font-semibold uppercase text-[var(--color-muted)]">{shortDay(item.day)}</p><p className="text-[9px] text-[#252F37]">{item.available} libres</p></div>
              })}
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2"><Legend color="#252F37" label="Disponible" /><Legend color="#681126" label="Reservada" /><Legend color="#d3a347" label="Hold pendiente" /><Legend color="#b85f54" label="Bloqueada" /></div>
        </article>
      </section>
      ...
      <section className="overflow-hidden rounded-xl border border-[var(--color-line)] bg-[var(--color-panel)] shadow-[var(--shadow-card)]">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--color-line)] px-4 py-3"><div><h2 className="text-base font-semibold">Calendario por cabaña</h2><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1"><Legend color="#252F37" label="Disponible" /><Legend color="#681126" label="Reservada" /><Legend color="#d3a347" label="Hold" /><Legend color="#b85f54" label="Bloqueada / fuera de servicio" /></div></div><span className="text-xs text-[var(--color-muted)]">{days.length} noches visibles</span></header>
