...
export function CartsPage() {
  ...
  return (
    <div className="control-page control-page--carts space-y-6">
      ...
      {selected ? <aside className="fixed inset-y-0 right-0 z-[var(--control-z-drawer)] flex w-full max-w-lg flex-col border-l border-[var(--color-line)] bg-[var(--color-panel)] shadow-2xl"><header className="flex items-start justify-between gap-4 border-b border-[var(--color-line)] p-5"><div className="min-w-0"><p className="text-xs font-medium uppercase tracking-[0.12em] text-[var(--color-muted)]">Carrito</p><h2 className="mt-1 break-words text-2xl text-[var(--color-ink)]" style={{ fontFamily: 'var(--font-display)' }}>{selected.customerName ?? 'Sesión sin identificar'}</h2><p className="mt-1 text-sm text-[var(--color-muted)]">{cartStatusLabel(selected.status)} · {inactivity(selected.inactiveMinutes)} sin actividad</p></div><button type="button" onClick={closeCart} className="shrink-0 rounded-full p-2 text-[var(--color-ink)]" aria-label="Cerrar detalle"><X size={20} /></button></header><div className="min-h-0 flex-1 space-y-6 overflow-y-auto p-5"><section><h3 className="text-sm font-medium text-[var(--color-ink)]">Partidas</h3><div className="mt-3 divide-y divide-[var(--color-line)] rounded-lg border border-[var(--color-line)]">{selected.items.map((item) => <div key={item.id} className="flex flex-wrap justify-between gap-3 p-3 text-sm"><span className="min-w-0 break-words">{item.name} · {item.quantity}</span><span className="shrink-0 font-semibold">{money(item.subtotal, item.currency)}</span></div>)}</div><p className="mt-3 text-right text-sm font-medium text-[var(--color-ink)]">{money(selected.estimatedValue, selected.currency)}</p>{selected.relatedOrderId ? <a href={`/control/ordenes?orderId=${encodeURIComponent(selected.relatedOrderId)}`} className="mt-3 inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-[var(--color-burgundy)] px-4 text-sm font-semibold text-[#fffaf1] hover:brightness-110" style={{ color: '#fffaf1' }}>Ver orden <ExternalLink size={14} aria-hidden="true" /></a> : <p className="mt-2 text-xs text-[var(--color-muted)]">Aún no existe una orden relacionada para este carrito.</p>}</section><section><h3 className="text-sm font-medium text-[var(--color-ink)]">Actividad relevante</h3><div className="mt-3 space-y-2">{selected.events.length ? selected.events.map((event) => <div key={event.id} className="rounded-md bg-[var(--color-soft)] p-3 text-sm"><p className="text-[var(--color-ink)]">{cartEventLabel(event.eventName)}</p><p className="mt-1 text-xs text-[var(--color-muted)]">{new Intl.DateTimeFormat('es-MX', { dateStyle: 'medium', timeStyle: 'short', timeZone: MEXICO_TIME_ZONE }).format(new Date(event.occurredAt))}</p></div>) : <p className="text-sm text-[var(--color-muted)]">Sin actividad adicional registrada.</p>}</div></section></div></aside> : null}
    </div>
  )
}
