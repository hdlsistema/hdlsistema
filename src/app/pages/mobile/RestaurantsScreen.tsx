    ...
    <div className="space-y-5 px-[var(--app-pad)] pb-8 pt-5">
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <article key={item.id} className={`w-full overflow-hidden rounded-[18px] border ${selectedRestaurant === item.id ? 'border-[#681126] bg-[#FFF5EA]' : 'border-[rgba(180,138,85,0.32)] bg-[#FFFDF8]'}`}>
      ...
      <section className="rounded-[20px] border border-[rgba(180,138,85,0.32)] bg-[var(--color-panel)] p-4">
        ...
        <div className="mt-3 rounded-[16px] border border-[rgba(180,138,85,0.32)] bg-[rgba(247,242,234,0.74)] p-3">
          ...
          {restaurantContractTerms.requiresAcceptance ? (
            <button
              ...
              className={`mt-3 flex min-h-11 w-full items-center justify-between gap-3 rounded-[14px] px-3 text-left text-[12px] font-semibold transition ${contractAccepted ? 'bg-[#681126] text-white' : 'bg-white/72 text-[var(--color-ink)] shadow-[inset_0_0_0_1px_rgba(180,138,85,0.22)]'}`}
            >
        ...
        <button type="button" onClick={submit} disabled={submitting || !canRequest} className="mt-3 flex min-h-12 w-full items-center justify-center rounded-full bg-[#681126] px-5 text-[14px] font-semibold text-white disabled:opacity-60">
