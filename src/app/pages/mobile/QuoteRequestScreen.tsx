...
function ChoiceSheet({
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-end bg-[rgba(45,24,17,0.28)] px-3 pb-[calc(env(safe-area-inset-bottom)+10px)] backdrop-blur-[6px]">
      <section className="max-h-[72dvh] w-full overflow-hidden rounded-[24px] border border-[rgba(180,138,85,0.42)] bg-[rgba(255,249,241,0.96)] shadow-[0_24px_54px_rgba(45,24,17,0.24)] backdrop-blur-2xl">
        <header className="flex items-center justify-between gap-3 border-b border-[rgba(180,138,85,0.32)] px-5 py-4">
          <h2 className="text-[18px] leading-none text-[var(--color-ink)]" style={{ fontFamily: 'var(--font-display)' }}>
            {title}
          </h2>
          <button type="button" onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full bg-white/70 text-[var(--color-ink)]" aria-label={closeLabel}>
            <X size={17} />
          </button>
        </header>
            ...
            return (
              <button
                ...
                className={`mb-2 flex min-h-[54px] w-full items-center justify-between gap-3 rounded-[18px] px-4 py-3 text-left ${
                  active
                    ? 'bg-[var(--color-burgundy)] text-white shadow-[0_12px_26px_rgba(45,5,17,0.22)]'
              ...
              >
                <span className="min-w-0">
                  ...
                  {option.description ? <span className={`mt-0.5 block text-[11px] leading-4 ${active ? 'text-white/78' : 'text-[var(--color-muted)]'}`}>{option.description}</span> : null}
                </span>
                <span className={`h-3 w-3 shrink-0 rounded-full ${active ? 'bg-[#B48A55]' : 'bg-[rgba(180,138,85,0.32)]'}`} />
              </button>
            )
          })}
        </div>
      ...
      <section className="rounded-[22px] border border-[rgba(180,138,85,0.32)] bg-[var(--color-panel)] p-4">
        <div className="grid grid-cols-2 gap-2">
          {(['social', 'business'] as const).map((value) => (
            <button key={value} type="button" onClick={() => setCategory(value)} className={`min-h-11 rounded-full text-[13px] font-semibold ${category === value ? 'bg-[var(--color-burgundy)] text-white' : 'bg-white/70 text-[var(--color-burgundy)]'}`}>
        ...
        {loading ? (
          ...
          >
            <span className="flex min-w-0 items-center gap-3">
              <MapPinned size={18} className="shrink-0 text-[var(--color-burgundy)]" />
              <span className="min-w-0">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <CrystalDateField value={preferredDate} onChange={setPreferredDate} placeholder={isEnglish ? 'Preferred date' : 'Fecha ideal'} buttonClassName="rounded-[16px] border-[rgba(180,138,85,0.42)] bg-white/70 text-[12px]" />
          <input
            type="number"
            inputMode="numeric"
            min={1}
            ...
            className="rounded-[16px] border border-[rgba(180,138,85,0.42)] bg-white/70 px-3 py-3 text-[13px] outline-none"
          />
        </div>
        <div className="mt-3 grid grid-cols-2 gap-3">
          <input placeholder={isEnglish ? 'First name' : 'Nombre'} value={firstName} onChange={(event) => setFirstName(event.target.value)} className="rounded-[16px] border border-[rgba(180,138,85,0.42)] bg-white/70 px-3 py-3 text-[13px] outline-none" />
          <input placeholder={isEnglish ? 'Last name' : 'Apellido'} value={lastName} onChange={(event) => setLastName(event.target.value)} className="rounded-[16px] border border-[rgba(180,138,85,0.42)] bg-white/70 px-3 py-3 text-[13px] outline-none" />
        </div>
