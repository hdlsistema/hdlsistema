...
const socialTypesEn = ['Wedding', 'Marriage proposal', 'Quinceañera', 'Baptism', 'First Communion', 'Baby shower', 'Birthday', 'Anniversary', 'Holiday gathering', 'Family gathering', 'Photo session', 'Other']
const businessTypesEn = ['Business dinner', 'Team building', 'Year-end event', 'Other']
    ...
    <div className="fixed inset-0 z-[150] flex items-end bg-[rgba(45,24,17,0.28)] px-3 pb-[calc(env(safe-area-inset-bottom)+10px)] backdrop-blur-[6px]">
      <section className="max-h-[72dvh] w-full overflow-hidden rounded-[24px] border border-[#E2CCAE] bg-[rgba(255,249,241,0.96)] shadow-[0_24px_54px_rgba(45,24,17,0.24)] backdrop-blur-2xl">
        <header className="flex items-center justify-between gap-3 border-b border-[#EBDCC8] px-5 py-4">
          ...
          <button type="button" onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full bg-white/70 text-[#2D1811]" aria-label={closeLabel}>
            <X size={17} />
          </button>
        </header>
...
export function QuoteRequestScreen() {
  ...
  const submit = async () => {
    if (!isAuthenticated) {
      setMessage(isEnglish ? 'Sign in to submit your request.' : 'Inicia sesión para enviar tu solicitud.')
      return
    }
    if (!firstName || !lastName || !phone) {
      setMessage(isEnglish ? 'Complete your first name, last name and phone.' : 'Completa nombre, apellido y teléfono.')
      return
    }
    setSubmitting(true)
    setMessage('')
    try {
      ...
      setMessage(`${isEnglish ? 'Request received' : 'Solicitud registrada'}: ${response.data.quoteNumber}`)
    } catch {
      setMessage(isEnglish ? 'We could not submit the request.' : 'No fue posible enviar la solicitud.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const translatedTypes = category === 'social' ? socialTypesEn : businessTypesEn
  const eventOptions = currentTypes.map((value, index) => ({ value, label: isEnglish ? translatedTypes[index] ?? value : value }))
  const selectedEventLabel = eventOptions.find((option) => option.value === eventType)?.label ?? eventType
  ...
  return (
    <div className="space-y-5 px-[var(--app-pad)] pb-8 pt-5">
      <header>
        <p className="text-[10px] font-semibold uppercase text-[#B88A4A]">{isEnglish ? 'Celebrate at Hacienda' : 'Celebra en Hacienda'}</p>
        <h1 className="mt-1 text-[clamp(26px,7vw,34px)] font-medium leading-none text-[#2D1811]" style={{ fontFamily: 'var(--font-display)' }}>
          {isEnglish ? 'Make Hacienda de Letras the setting for your next story.' : 'Haz de Hacienda de Letras el escenario de tu próxima historia.'}
        </h1>
      </header>
      ...
      <section className="rounded-[22px] border border-[#EBDCC8] bg-[#FFF9F1] p-4">
        ...
        >
          <span>
            <span className="block text-[10px] font-semibold uppercase tracking-[0.16em] text-[#B88A4A]">{isEnglish ? 'Event type' : 'Tipo de evento'}</span>
            <span className="mt-1 block text-[15px] font-semibold text-[#2D1811]">{selectedEventLabel}</span>
          </span>
          <ChevronDown size={18} className="shrink-0 text-[#8A1238]" />
        </button>
        {loading ? (
          <div className="mt-4 rounded-[16px] border border-[#E2CCAE] bg-white/70 px-3 py-3 text-[12px] text-[#690D2B]">{isEnglish ? 'Loading spaces...' : 'Cargando espacios...'}</div>
        ) : error ? (
          <div className="mt-4"><ErrorState message={error} retryLabel={isEnglish ? 'Try again' : 'Reintentar'} onRetry={retry} /></div>
        ) : spaces.length === 0 ? (
          <div className="mt-4"><EmptyState title={isEnglish ? 'No spaces published' : 'Sin espacios publicados'} description={isEnglish ? 'You will soon be able to choose the perfect space for your celebration.' : 'Pronto podrás elegir el espacio ideal para tu celebración.'} /></div>
        ) : (
          <button
            type="button"
          ...
          >
            <span className="flex min-w-0 items-center gap-3">
              ...
              <span className="min-w-0">
                <span className="block text-[10px] font-semibold uppercase tracking-[0.16em] text-[#B88A4A]">{isEnglish ? 'Space' : 'Espacio'}</span>
                <span className="mt-1 block truncate text-[15px] font-semibold text-[#2D1811]">{selectedSpaceRecord?.name ?? (isEnglish ? 'Choose space' : 'Elegir espacio')}</span>
              </span>
            </span>
            <ChevronDown size={18} className="shrink-0 text-[#8A1238]" />
          </button>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <CrystalDateField value={preferredDate} onChange={setPreferredDate} placeholder={isEnglish ? 'Preferred date' : 'Fecha ideal'} buttonClassName="rounded-[16px] border-[#E2CCAE] bg-white/70 text-[12px]" />
          <input
            type="number"
            inputMode="numeric"
            min={1}
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          <input placeholder={isEnglish ? 'First name' : 'Nombre'} value={firstName} onChange={(event) => setFirstName(event.target.value)} className="rounded-[16px] border border-[#E2CCAE] bg-white/70 px-3 py-3 text-[13px] outline-none" />
          <input placeholder={isEnglish ? 'Last name' : 'Apellido'} value={lastName} onChange={(event) => setLastName(event.target.value)} className="rounded-[16px] border border-[#E2CCAE] bg-white/70 px-3 py-3 text-[13px] outline-none" />
        </div>
        <input placeholder={isEnglish ? 'Phone' : 'Teléfono'} value={phone} onChange={(event) => setPhone(event.target.value)} className="mt-3 w-full rounded-[16px] border border-[#E2CCAE] bg-white/70 px-3 py-3 text-[13px] outline-none" />
        <textarea placeholder={isEnglish ? 'Event notes' : 'Notas del evento'} value={notes} onChange={(event) => setNotes(event.target.value)} className="mt-3 min-h-24 w-full rounded-[16px] border border-[#E2CCAE] bg-white/70 px-3 py-3 text-[13px] outline-none" />
      ...
      {eventSheetOpen ? (
        <ChoiceSheet
          title={isEnglish ? 'Event type' : 'Tipo de evento'}
      ...
      {spaceSheetOpen ? (
        <ChoiceSheet
          title={isEnglish ? 'Event space' : 'Espacio para evento'}
